...
export function initScroll({ camera, reduced }) {
  ...
  gsap.timeline({ scrollTrigger: { trigger: '#act-explode', start: 'top 80%', end: 'bottom bottom', scrub } })
    .fromTo(state, { explode: 0 }, { explode: 1, ease: 'none', immediateRender: false }, 0)
    .fromTo(camera.position, { x: 0, y: 0.6, z: 6.5 }, { x: 1.3, y: 1.7, z: 7.5, ease: 'none', immediateRender: false }, 0)
  ...
  gsap.timeline({ scrollTrigger: { trigger: '#act-flow', start: 'top bottom', end: 'center center', scrub } })
    .fromTo(state, { explode: 1, flow: 0 }, { explode: 0, flow: 1, ease: 'none', immediateRender: false }, 0)
    .fromTo(camera.position, { x: 1.3, y: 1.7, z: 7.5 }, { x: -1.5, y: 0.9, z: 8.5, ease: 'none', immediateRender: false }, 0)

  // Act 5: flow calms, camera pulls back
  gsap.timeline({ scrollTrigger: { trigger: '#act-specs', start: 'top bottom', end: 'top 30%', scrub } })
    .fromTo(state, { flow: 1 }, { flow: 0.25, ease: 'none', immediateRender: false }, 0)
    .fromTo(camera.position, { x: -1.5, y: 0.9, z: 8.5 }, { x: 0, y: 1.4, z: 10, ease: 'none', immediateRender: false }, 0)
